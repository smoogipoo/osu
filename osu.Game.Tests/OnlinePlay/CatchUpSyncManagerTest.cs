...
{
    [HeadlessTest]
    public class CatchUpSyncManagerTest : OsuTestScene
    {
        private TestManualClock master;
        private CatchUpSyncManager syncManager;

        private TestSyncSlaveClock slave1;
        private TestSyncSlaveClock slave2;

        [SetUp]
        public void Setup()
        {
            syncManager = new CatchUpSyncManager(master = new TestManualClock());
            syncManager.AddSlave(slave1 = new TestSyncSlaveClock(1));
            syncManager.AddSlave(slave2 = new TestSyncSlaveClock(2));
        ...
        private void setWaiting(Func<TestSyncSlaveClock> slave, bool waiting)
        ...
        /// </summary>
        private void setSlaveTime(Func<TestSyncSlaveClock> slave, double offsetFromMaster)
        ...
        private void assertCatchingUp(Func<TestSyncSlaveClock> slave, bool catchingUp) =>
            AddAssert($"slave {slave().Id} {(catchingUp ? "is" : "is not")} catching up", () => slave().IsCatchingUp == catchingUp);

        private void assertSlaveState(Func<TestSyncSlaveClock> slave, bool running)
            => AddAssert($"slave {slave().Id} {(running ? "is" : "is not")} running", () => slave().IsRunning == running);

        private class TestSyncSlaveClock : TestManualClock, ISyncSlaveClock
        {
            public readonly Bindable<bool> WaitingOnFrames = new Bindable<bool>(true);
            IBindable<bool> ISyncSlaveClock.WaitingOnFrames => WaitingOnFrames;
            public bool IsCatchingUp { get; set; }
            public readonly int Id;

            public TestSyncSlaveClock(int id)
            {
                Id = id;
