...
namespace osu.Game.Rulesets.Mania.Edit
{
    public class ManiaSelectionHandler : SelectionHandler
    {
        ...
        {
            int lastColumn = ((ManiaHitObject)blueprint.HitObject.HitObject).Column;
            Vector2 delta = blueprint.Parent.ToLocalSpace(screenSpacePosition) - blueprint.HitObject.Position;

            adjustOrigins((ManiaSelectionBlueprint)blueprint);
            performDragMovement(delta);
            performColumnMovement(lastColumn, screenSpacePosition);
        ...
        private void performDragMovement(Vector2 delta)
        {
        ...
        private void performColumnMovement(int lastColumn, Vector2 screenSpacePosition)
        {
            var currentColumn = composer.ColumnAt(screenSpacePosition);
